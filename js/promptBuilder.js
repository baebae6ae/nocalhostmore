...
export function buildPrompt(answers) {
  ...
  if (spec.aitool === 'agent') {
    ...
    tip =
      '이 프롬프트를 코딩 에이전트(Cursor·Claude Code 등)에 그대로 붙여넣으면 파일을 직접 만듭니다. 먼저 제시하는 "구조 계획"을 확인한 뒤 진행을 승인하세요.';
  } else {
    protocolBlock =
    ...
    tip =
      '붙여넣은 뒤 AI가 멈추면 "다음"이라고 입력해 이어받으세요. 코드가 중간에 잘리면 "이어서"라고 하면 됩니다. 파일이 많은 프로젝트라면 Cursor·Claude Code 같은 에이전트형이 더 편합니다.';
  }

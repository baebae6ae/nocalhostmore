...
export const QUESTIONS = [
  ...
  // ── 2. 어떻게 쓸 것인가 (진짜 분기점) ───────────────────────────
  {
    ...
    recommend: 'deploy',
    recommendReason:
      '"언젠가 남한테 링크로 보여줄 수도 있다"는 생각이 조금이라도 있으면 처음부터 "인터넷에 올린다"로 설계하는 게 안전합니다. 혼자용으로 짠 코드를 나중에 여럿용으로 바꾸는 건 사실상 처음부터 다시 만드는 일이거든요. 정말 순수 연습·개인 도구면 첫 번째를 고르세요.',
  },
  ...
  // ── 3. (배포 시) 누가 어떻게 쓰나 — 모순 축을 하나로 묶음 ────────
  {
    ...
    recommend: 'tool',
    recommendReason:
      '첫 서비스라면 "로그인 없는 도구"(첫 번째)가 가장 쉽고, 대부분 무료로 배포할 수 있어 추천합니다. ' +
      '"사람마다 자기 기록이 있어야 한다"면 두 번째(로그인+내 데이터), ' +
      '"모두가 같은 걸 함께 본다(게시판·순위표)"면 세 번째예요. ' +
      '⚠️ 두 번째·세 번째는 데이터를 계속 저장할 서버(DB)가 필요해 무료 범위를 넘어 요금이 들 수 있습니다.',
  },
  ...
  // ── 4. 파일 업로드 ─────────────────────────────────────────────
  {
    ...
    recommend: 'no',
    recommendReason:
      '지금 당장 파일 올리는 기능이 확실히 없으면 "아니요"를 고르세요. 나중에 필요해지면 그때 추가해도 늦지 않습니다.',
  },
  {
    id: 'fileHandling',
    step: 'data',
    ...
    recommend: 'ephemeral',
    recommendReason:
      '"올려서 한 번 처리하고 결과만 보여주면 끝"이면 일회성(첫 번째)이 안전하고 비용도 안 듭니다. ' +
      '"내가 올린 목록을 다시 본다"가 핵심 기능일 때만 영구 보관을 고르세요. ⚠️ 영구 보관은 저장공간(클라우드) 비용이 들 수 있어요.',
  },
  ...
  // ── 5. (로컬 전용) 기록 저장 여부 ──────────────────────────────
  {
    ...
    recommend: 'memory',
    recommendReason:
      '정말로 "결과 한 번 보여주고 끝"이면 저장 없이 간단하게(첫 번째) 가세요. 히스토리·누적 기록이 핵심이면 저장(두 번째)을 고르면 됩니다.',
  },
